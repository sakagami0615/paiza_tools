...
class CodeGenerator:
    ...
    def __init__(self, root_dir: str):
        ...
        self.__check_file_not_exist(config_filepath)
        with open(config_filepath) as f:
            self.config = yaml.safe_load(f)

    def __check_file_exist(self, filepath: str) -> None:
        if os.path.isfile(filepath):
            raise FileExistsError

    def __check_file_not_exist(self, filepath: str) -> None:
        if not os.path.isfile(filepath):
            raise FileNotExistsError

    def __render_template(self, template_dir_path: str, template_file_name: str,
                          render_param_dict: dict) -> str:
        template_env = Environment(loader=FileSystemLoader(template_dir_path, encoding='utf8'))
    ...
    def generate_file(self, content: QuestionContent, dirpath: str, is_overwrite: bool) -> None:
        ...
        self.__check_file_not_exist(meta_file_path)
        ...
        if not is_overwrite:
            self.__check_file_exist(script_file_path)

        # Get template file path. In addition, check for existence.
        template_file_path = os.path.join(self.root_dir, self.config['Template']['FilePath'])
        self.__check_file_not_exist(template_file_path)
        ...
        render_param_dict = ExtractRenderParam(self.config, content).extract_param_dict()
        render_code = self.__render_template(template_dir_path, template_file_name,
                                             render_param_dict)
