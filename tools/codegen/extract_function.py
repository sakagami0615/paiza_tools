...
class ExtractFunction:
    ...
    @staticmethod
    def extract_variable_datatype(var_dict: dict, onecase_input_list: List[str]) -> dict:
        var_datatype_dict = {}
        var_num_dict = {}
                ...
                if type(begin_line) == int and type(end_line) == int:
                    ...
                    for var in vars:
                        ...
                        if token_idx < len(input_tokens):
                            ...
                            var_datatype_dict[var.name] = datatype(input_tokens[token_idx])
                        # 次の探索データまでインデックスを進める
    ...
    @staticmethod
    def create_variable_dict(var_format_list: List[str], onecase_input_list: List[str]) -> dict:
        ...
        for line in var_format_list:
            for item in line.split():
                if '...' in item:
                    continue
                var_name, size_1d, size_2d = ExtractFunction.extract_varname_and_size(item)
                # 日本語などの説明などは除外する
        ...
        datatype_dict = ExtractFunction.extract_variable_datatype(var_dict, onecase_input_list)
        for key in var_dict:
    ...
    @staticmethod
    def create_input_process_str(var_dict: dict, stdin_proc_code: str, tab_str: str = '    ') -> str:
        input_proc_str = ''
        curr_indent = 1
        ...
        for vars in var_dict.values():
            ...
            for var in vars:
                ...
                # 2d list
                    ...
                    list_name = f"{var.name}_list"
                    input_proc_str += tab_str * curr_indent
                    input_proc_str += f"{list_name} = [[] * i for i in range({var.size_1d})]\n"
            ...
            for var in vars:
                ...
                if var.size_1d == 1:
                    # inputコード
                    input_proc_str += tab_str * curr_indent
                    input_proc_str += f"{var.name} = {var.datatype}({stdin_proc_code})\n"
                # 1d list
                elif var.size_2d == 1:
                    # for文コード
                    if for_cnt < 1:
    ...
    def create_solve_argument_str(var_dict: dict) -> str:
        varname = lambda v: v.name if v.size_1d == 1 and v.size_2d == 1 else v.name + '_list'
        solve_arg_str = ', '.join([varname(value) for values in var_dict.values() for value in values])
