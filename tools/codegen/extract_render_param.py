...
class ExtractRenderParam:
    ...
    def __extract_code(self):
        # 変数抽出用の入力を用意
        var_format = ExtractFunction.remove_blank_in_curly_brackets(self.content.var_format)
        var_format_list = var_format.split('\n')

        # 変数情報を格納した辞書を作成
        onecase_input_list = [] if self.content.n_test_cases == 0 else self.content.input_list[0].split('\n')
        var_dict = ExtractFunction.create_variable_dict(var_format_list, onecase_input_list)
        ...
        input_process_str = ExtractFunction.create_input_process_str(var_dict,
                                                                     self.config['Template']['StdinProcCode'],
                                                                     self.config['TabString'])
