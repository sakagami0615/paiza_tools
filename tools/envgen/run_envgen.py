...
class RunEnvGen:
    ...
    def __get_cmdline_args(self, prog: str, args: List[str]) -> argparse.Namespace:
        ...
        parser.add_argument('-w', '--workspace', default='./',
                            help="Path to create the environment."
    ...
    def run(self):
        try:
            ...
            EnvFileGenerator().generate_file(content, self.args.workspace, self.args.overwrite)

            ques_dirpath = os.path.join(self.args.workspace, content.ques_number)
            CodeGenerator(self.root_dir).generate_file(content, ques_dirpath, self.args.overwrite)
